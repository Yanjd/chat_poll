	...
	while(1)
	{
		...
		for(int i=0;i<user_count+1;++i)
		{
			...
			else if(fds[i].revents & POLLIN)
			{
				...
				ret=recv(connfd,users[connfd].buf,BUF_SIZE-1,0);
				printf("-----get %d bytes of data from %d\nthe content is \n%s\n",ret,connfd,users[connfd].buf);
				if(ret<0)
				{
					if(errno!=EAGAIN)
					{
				...
				else
				{
					...
					for(int j=1;j<=user_count;++j)
					{
						...
						//printf("here send\n");
						//send(fds[j].fd,users[connfd].buf,BUF_SIZE,0);
						fds[j].events |= ~POLLIN;
